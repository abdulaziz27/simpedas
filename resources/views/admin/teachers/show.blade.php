    ...
    <div class="bg-[#09443c] p-10 rounded-2xl shadow-lg print:bg-white print:text-black">
        ...
        <div class="flex flex-col md:flex-row items-center md:items-start gap-8 mb-8 pb-8 border-b border-gray-700 print:border-gray-300">
            <div class="flex-shrink-0">
                <div class="h-44 w-44 bg-gray-300 flex items-center justify-center border-4 border-gray-200 rounded-xl overflow-hidden">
                    @if($teacher->photo)
                        <img src="{{ asset('storage/' . $teacher->photo) }}" alt="Foto {{ $teacher->full_name }}" class="h-44 w-44 object-cover rounded-xl">
                    @else
        ...
        <div class="mt-8 pt-8 border-t border-gray-700 print:border-gray-300 print:hidden">
            <div class="flex flex-wrap gap-4 justify-center">
                <a href="{{ auth()->user()->hasRole('admin_sekolah') ? route('sekolah.teachers.edit', $teacher->id) : route('dinas.teachers.edit', $teacher->id) }}"
                ...
                <a href="{{ auth()->user()->hasRole('admin_sekolah') ? route('sekolah.teachers.print', $teacher) : route('dinas.teachers.print', $teacher) }}"
                   target="_blank"
                ...
                <button onclick="openDeleteModal('teacher', {{ $teacher->id }})"
            ...
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">Hapus Data Guru</h3>
                        <div class="mt-2">
                            <p class="text-sm text-gray-500" id="modal-message">Apakah Anda yakin ingin menghapus data guru ini? Tindakan ini tidak dapat dibatalkan.</p>
                        </div>
                    </div>
                </div>
            </div>
...
document.getElementById('confirmDelete').addEventListener('click', function() {
    if (deleteType === 'teacher') {
        ...
        form.method = 'POST';
        form.action = '{{ auth()->user()->hasRole("admin_sekolah") ? route("sekolah.teachers.destroy", ":id") : route("dinas.teachers.destroy", ":id") }}'.replace(':id', deleteId);
