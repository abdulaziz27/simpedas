...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (statistikChart) {
        ...
        const chartOptions = JSON.parse(
            statistikChart.dataset.chartOptions || "{}"
        );
    ...
    if (statistikBarChart) {
        const barChartData = JSON.parse(
            statistikBarChart.dataset.chartData || "{}"
        );
        const barChartOptions = JSON.parse(
            statistikBarChart.dataset.chartOptions || "{}"
        );
        ...
        barChartOptions.plugins.datalabels = {
            ...
            formatter: (value, context) => {
                ...
                if (context.datasetIndex === 0) {
                    ...
                    return percentages[context.dataIndex] !== undefined
        ...
        // Fix tooltip callbacks for bar chart
        if (
            barChartOptions.plugins &&
            barChartOptions.plugins.tooltip &&
            barChartOptions.plugins.tooltip.callbacks
        ) {
            barChartOptions.plugins.tooltip.callbacks.label = function (
                context
            ) {
